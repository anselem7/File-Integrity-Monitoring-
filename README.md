...
Ensuring data remains accurate, consistent, and unaltered unless modified by authorized users.
...
File Integrity Monitoring tracks critical files and detects unauthorized changes by comparing current file hashes against a known trusted baseline.

Hashing

A cryptographic hash acts as a digital fingerprint for a file. Even a minor modification to a file produces a significantly different hash value, making hashing an effective method for integrity verification.
...
User-configurable monitoring paths.
Email notifications.
SMS or API-based alerts.
Logging to files or databases.
Real-time monitoring using file system watchers.
Improved exception handling.
Dashboard integration for event visualization.
Limitations
No support for network drives.
No alerting beyond console output.
Limited error handling.
Baseline hashes are stored in plain text.
Performance optimization for large directories has not been implemented.
Assumes the baseline represents a trusted system state.
Learning Outcomes
